...
logger = logging.getLogger("qna_ai_service")
...
logger.debug("Loaded QnA service parameters successfully")

def init_api_client():
    global client
    client = None

    logger.debug("Initializing IBM watsonx.ai API client")
    # init API client
    try:
        if parameters['watsonx_ai_api_key']:
            ...
            logger.debug("WML credentials prepared for environment")
            client = APIClient(wml_credentials)
            logger.info("Initialized IBM watsonx.ai APIClient")

            if parameters['watsonx_space_id']:
                logger.debug("Default space set to: %s", parameters['watsonx_space_id'])
