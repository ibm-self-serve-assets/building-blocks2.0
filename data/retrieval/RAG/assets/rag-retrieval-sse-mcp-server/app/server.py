...
def _format_hits(hits: List[Dict[str, Any]]) -> List[SearchResult]:
    ...
    for h in hits:
        ...
        chunk_seq_val = src.get("chunk_seq", "")
        out.append(
            SearchResult(
                ...
                page_number=str(src.get("page_number", "")),
                chunk_seq=str(chunk_seq_val) if chunk_seq_val else "",
...
async def retrieve(request: RetrievalRequest):
    ...
    if not emb_cfg.is_configured():
        raise HTTPException(
            status_code=500,
            detail="Embedding config missing. Set IBM_API_KEY (or WATSONX_API_KEY), WATSONX_URL, WATSONX_PROJECT_ID, EMBEDDING_MODEL_ID."
        )
    if not vdb.is_configured():
        raise HTTPException(
            status_code=500,
