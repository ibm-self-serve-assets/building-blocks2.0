#!/usr/bin/env bash
# =============================================================================
# deploy-mcp-server.sh — Maximo Knowledge Hub MCP Server
#
# Builds the container image from deployment/Dockerfile.mcp-server,
# pushes it to IBM Cloud Container Registry (ICR), and creates or
# updates the MCP server application on IBM Cloud Code Engine.
#
# USAGE
#   ./deployment/deploy-mcp-server.sh [OPTIONS]
#
# OPTIONS
...
#   WATSONX_MODEL_ID          LLM model ID              (default: ibm/granite-13b-chat-v2)
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/deploy-mcp-server-$(date '+%Y%m%d-%H%M%S').log"
...
ICR_IMAGE_NAME="${ICR_IMAGE_NAME:-mcp-server}"
...
WATSONX_MODEL_ID="${WATSONX_MODEL_ID:-meta-llama/llama-3-3-70b-instruct}"
WATSONX_EMBEDDING_MODEL_ID="${WATSONX_EMBEDDING_MODEL_ID:-ibm/slate-30m-english-rtrvr-v2}"
CORS_ORIGIN="${CORS_ORIGIN:-}"
# Maximo Manage connection (optional — leave blank to disable live Maximo queries)
...
MAXIMO_VERIFY_SSL="${MAXIMO_VERIFY_SSL:-false}"
# ServiceNow connection (optional — leave blank to disable ServiceNow queries)
SERVICENOW_INSTANCE_URL="${SERVICENOW_INSTANCE_URL:-}"
SERVICENOW_USERNAME="${SERVICENOW_USERNAME:-}"
SERVICENOW_PASSWORD="${SERVICENOW_PASSWORD:-}"
SERVICENOW_VERIFY_SSL="${SERVICENOW_VERIFY_SSL:-1}"
# Spiderbot web-knowledge index (optional — leave blank for default)
SPIDERBOT_INDEX="${SPIDERBOT_INDEX:-maximo_web_knowledge}"
...
run_local() {
  ...
  ${RUNTIME} build \
    --file "${REPO_ROOT}/deployment/Dockerfile.mcp-server" \
    --tag  "${image}" \
    "${REPO_ROOT}"
  ...
  ${RUNTIME} run -d \
    ...
    -e SERVICENOW_VERIFY_SSL="${SERVICENOW_VERIFY_SSL}" \
    -e SPIDERBOT_INDEX="${SPIDERBOT_INDEX}" \
  ...
  if [[ "$status" == "running" ]]; then
    success "Container is running — MCP Server at http://localhost:${LOCAL_PORT}"
...
deploy_to_ce() {
  ...
  "${IBMCLOUD}" ce secret create \
    ...
    --from-literal "SERVICENOW_VERIFY_SSL=${SERVICENOW_VERIFY_SSL}" \
    --from-literal "SPIDERBOT_INDEX=${SPIDERBOT_INDEX}" \
    --from-literal "MAXIMO_URL=${MAXIMO_URL}" \
    --from-literal "MAXIMO_API_KEY=${MAXIMO_API_KEY}" \
    --from-literal "MAXIMO_USERNAME=${MAXIMO_USERNAME}" \
    --from-literal "MAXIMO_PASSWORD=${MAXIMO_PASSWORD}" \
    --from-literal "MAXIMO_TIMEOUT=${MAXIMO_TIMEOUT}" \
    --from-literal "MAXIMO_VERIFY_SSL=${MAXIMO_VERIFY_SSL}" \
  ...
  if [[ -n "${app_url}" ]]; then
    local url_file="${SCRIPT_DIR}/.deploy-url-mcp-server"
...
# MAIN
# =============================================================================
header "Maximo Knowledge Hub — MCP Server Deploy"
